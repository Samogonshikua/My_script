...
RETRY = 999999999999 # кол-во попыток при ошибках / фейлах
...
MAX_GWEI    = 18 # gas в gwei (смотреть здесь : https://etherscan.io/gastracker)
...
def value_transfer():
    ...
    chain                = 'zksync' # в какой сети выводить
    token_address        = '' # пусто если нативный токен сети  0xBe9895146f7AF43049ca1c1AE358B0541Ea49704

    amount_from          = 0.0045 # от какого кол-ва монет делаем трансфер
    amount_to            = 0.005 # до какого кол-ва монет делаем трансфер
    ...
    keep_value_from      = 0.0006 # от скольки монет оставляем на кошельке (работает только при : transfer_all_balance = True)
    keep_value_to        = 0.0006 # до скольки монет оставляем на кошельке (работает только при : transfer_all_balance = True)
...
def value_bungee():
    ...
    amount_from         = 0.0034 # от какого кол-ва монет делаем бридж
    amount_to           = 0.0034 # до какого кол-в монет делаем бридж

    bridge_all_balance  = True # True / False. если True, тогда бриджим весь баланс
    min_amount_bridge   = 0 # если баланс будет меньше этого числа, выводить не будет
    keep_value_from     = 0.00062 # от скольки монет оставляем на кошельке (работает только при : bridge_all_balance = True)
    keep_value_to       = 0.00062 # до скольки монет оставляем на кошельке (работает только при : bridge_all_balance = True)
...
def value_1inch_swap():
    ...
    from_token_address  = '' # пусто если нативный токен сети
    to_token_address    = '0x5aea5775959fbc2557cc8789bc1bf90a239d9a91' # пусто если нативный токен сети
...
def value_merkly_refuel():
    ...
    from_chains = ['polygon'] # запиши сюда сети, с которых хочешь делать refuel (>= 1 сети)
    to_chains   = ['gnosis'] # запиши сюда сети, на которые хочешь делать refuel (>= 1 сети)
